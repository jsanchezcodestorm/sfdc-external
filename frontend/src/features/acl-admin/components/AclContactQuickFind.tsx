...
import { fetchAclContactSuggestions } from '../acl-admin-api'
import type { AclAdminContactSuggestion } from '../acl-admin-types'

type AclContactQuickFindProps = {
  value: string
  onChange: (value: string) => void
  onSelect: (suggestion: AclAdminContactSuggestion) => void
  placeholder?: string
}

export function AclContactQuickFind({
  value,
  onChange,
  onSelect,
  placeholder,
}: AclContactQuickFindProps) {
  const [suggestions, setSuggestions] = useState<AclAdminContactSuggestion[]>([])
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      setLoading(true)
      setError(null)
      void fetchAclContactSuggestions(searchValue, 8)
        .then((payload) => {
          if (cancelled) {
            return
          }
